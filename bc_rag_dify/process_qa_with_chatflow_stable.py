...
def process_all_qa_files():
    ...
    # 获取前20个qa文件（减少数量，提高成功率）
    qa_dir = Path('breast_cancer_cmed_files')
    qa_files = sorted([f for f in qa_dir.glob('qa*.json')])[:20]
    
    print(f"🚀 开始并发处理 {len(qa_files)} 个QA文件...")
    print(f"🔧 使用3个并发线程（减少并发数）")
    ...
    if all_scores:
        ...
        summary_filepath = output_dir / "summary_results_stable.json"
        with open(summary_filepath, 'w', encoding='utf-8') as f:
            json.dump(summary_data, f, ensure_ascii=False, indent=2)
        
        print(f"📄 汇总结果已保存到: summary_results_stable.json")
    else:
        print("❌ 没有成功处理的文件")
