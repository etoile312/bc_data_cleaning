...
def process_single_case(case: Dict[str, Any], case_id: int) -> Dict[str, Any]:
    ...
    text_v2 = ask_model(prompt2)
    if text_v2:
        text_v2 = text_v2.replace("\\n", "\n")
        text_v2 = re.sub(r"。(?!(\n|$))", "。\n", text_v2)
    
    return {
        "structured": case_with_name,
        "text": text_v2 or text_v1 or ""
    }
...
def main():
    ...
    # 读取结构化病例数据
    try:
        with open("data/combined_cases.json", encoding="utf-8") as f:
            cases = json.load(f)
    except FileNotFoundError:
        print("错误：data/combined_cases.json文件不存在，请先运行combine_segments2.py生成结构化数据")
        return
    ...
    for idx, case in enumerate(cases):
        ...
        # 写入 JSON 文件
        json_file = f"data/generated_cases/case_{idx+1:03d}.json"
        with open(json_file, "w", encoding="utf-8") as f:
            json.dump(result, f, ensure_ascii=False, indent=2)

        # 写入 TXT 文件
        txt_file = f"data/generated_cases/case_{idx+1:03d}.txt"
